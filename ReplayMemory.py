...
class ReplayMemory:
    ...
    """
    def __init__(self, size, image_size, stack_size, minibatch_size):
        # Pre allocate
        self._memory_state = np.zeros(shape=(size, image_size[0], image_size[1], stack_size), dtype=np.int8)
        self._memory_future_state = np.zeros(shape=(size, image_size[0], image_size[1], stack_size), dtype=np.int8)
        self._rewards = np.zeros(shape=(size, 1), dtype=np.float32)
        self._done = np.zeros(shape=(size, 1), dtype=np.bool)
        self._actions = np.zeros(shape=(size, 1), dtype=np.int8)

        self._mini_batch_state = np.zeros(shape=(minibatch_size, image_size[0], image_size[1], stack_size), dtype=np.float32)
        self._mini_batch_future_state = np.zeros(shape=(minibatch_size, image_size[0], image_size[1], stack_size), dtype=np.float32)
    ...
    def add_example(self, state, future_state, action, reward, is_terminal):
        ...
        self._done[position] = is_terminal
